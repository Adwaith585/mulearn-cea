...
const defaultCoreMembers: Application[] = [
    ...
    { id: "4", name: "Abhin J Gomez", role: "Design Lead", domains: ["IOT And Robotics", "Human Resources"], karma: 2204, imageUrl: "/leads/abhinjgomez.jpg", isCore: true, status: "approved", github: "https://github.com/Abhin147", linkedin: "https://www.linkedin.com/in/abhin-j-gomez/", instagram: "" },
...
export function getApplications(): Application[] {
    ...
    // Use proper array spread to unshift in correct order instead of looping backwards
    const missingCores = defaultCoreMembers.filter(core => !existingIds.has(core.id));
    if (missingCores.length > 0) {
        parsed.unshift(...missingCores);
        hasChanges = true;
    }

    // Always enforce strict sorting so Leads appear exactly in designated order
    parsed.sort((a, b) => {
        const indexA = defaultCoreMembers.findIndex(c => c.id === a.id);
        const indexB = defaultCoreMembers.findIndex(c => c.id === b.id);

        // Both are core members -> sort by their hardcoded hierarchy
        if (indexA !== -1 && indexB !== -1) return indexA - indexB;
        // Only A is core -> A goes first
        if (indexA !== -1) return -1;
        // Only B is core -> B goes first
        if (indexB !== -1) return 1;

        // Neither are core -> sort by karma descending
