...
export function DocsList() {
    ...
    if (docs.length === 0) {
        return (
            <div className="py-20 text-center border border-dashed border-white/10 rounded-3xl bg-surface/20 flex flex-col items-center">
                <FileText className="w-12 h-12 mb-4 opacity-20" />
                <p className="text-xl font-heading font-medium text-white mb-2">No documents yet.</p>
    ...
    return (
        <div className="space-y-12">
            {docs.map(doc => (
                <Card key={doc.id} className="bg-surface/30 border-white/5 overflow-hidden transition-all hover:border-primary/30 group">
                    {doc.imageUrl && (
