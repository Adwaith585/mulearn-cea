        ...
        return (
            <div className="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center max-w-2xl mx-auto px-4 space-y-8 animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <h1 className="text-5xl md:text-7xl font-heading font-bold text-text-primary tracking-tight">Projects</h1>
    ...
    return (
        <div className="container mx-auto px-4 py-24 max-w-7xl space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <section className="text-center space-y-4 max-w-2xl mx-auto">
                ...
                <h1 className="text-5xl md:text-7xl font-heading font-bold text-text-primary tracking-tight">Student Projects</h1>
                <p className="text-xl text-text-muted leading-relaxed">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {projects.map((project) => (
                    <Card key={project.id} className="bg-surface/30 hover:border-primary/30 transition-all hover:shadow-[0_0_30px_rgba(108,92,231,0.15)] group">
                        {project.imageUrl && (
                            <div className="w-full h-48 overflow-hidden rounded-t-xl relative border-b border-border">
                                ...
                                <div className="absolute bottom-4 left-4 z-20">
                                    <h3 className="text-2xl font-bold font-heading text-text-primary">{project.title}</h3>
                        ...
                        <CardContent className="p-6 space-y-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {project.techStack.map(tech => (
                                    <span key={tech} className="text-xs px-2.5 py-1 bg-background border border-border rounded-md text-text-muted font-mono">{tech}</span>
                                ))}
                            </div>

                            <div className="flex gap-4 pt-4 border-t border-border">
                                {project.repoUrl && (
                                    <a href={project.repoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-bold text-text-primary hover:text-primary transition-colors">
                                ...
                                {project.liveUrl && (
                                    <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-bold text-text-primary hover:text-karma transition-colors">
